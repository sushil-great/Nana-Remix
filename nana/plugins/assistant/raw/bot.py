...
from nana import ASSISTANT_BOT_TOKEN

from .Parser import mixed_to_html


class Bot:
    def __init__(self):
        self.api = 'https://api.telegram.org/bot' + ASSISTANT_BOT_TOKEN
        self._session: Optional[aiohttp.ClientSession] = None

    @staticmethod
    def get_new_session() -> aiohttp.ClientSession:
        return aiohttp.ClientSession(
            json_serialize=ujson.dumps, timeout=aiohttp.ClientTimeout(
                total=300,
            ),
        )
    ...
    async def edit_inline_text(
        ...
        if reply_markup:  # :: Optional ::
            params['reply_markup'] = Bot.InlineKeyboard(reply_markup)
    ...
    def InlineKeyboard(mkrp):
        ...
        return ujson.dumps({'inline_keyboard': Bot.clean_markup(buttons)})
...
class XMediaTypes:
    ...
    async def InputMediaDocument(
        ...
        if isinstance(disable_content_type_detection, bool):
            media['disable_content_type_detection'] = disable_content_type_detection  # noqa: E501
        if parse_mode.lower() in ('md', 'markdown'):
            media['parse_mode'] = 'Markdown'
        elif parse_mode.lower() in ('html', 'mixed'):
            media['parse_mode'] = 'HTML'
